...
import com.ca.mfd.prc.common.config.ApiBomPlatformConfig;
 ...
 * @version 2023年7月5日
 */
@Service("apiBomPtService")
public class IApiBomPtformService implements IApiPtformBaseService {

    @Autowired
    private ApiBomPlatformConfig apiBomPlatformConfig;

    @Autowired
    @Qualifier("apiPtBomRestTemplate")
    @Lazy
    private RestTemplate apiPtBomRestTemplate;

    public String getapi(String path, Object pms, Map<String, String> heads) {
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        headers.add("Accept", MediaType.APPLICATION_JSON.toString());
        headers.add("appKey", apiBomPlatformConfig.getApiAppKey());
        if (heads != null) {
        ...
        if (path.startsWith("http:") || path.startsWith("https:")) {
            ...
            urlsb.append(apiBomPlatformConfig.getPlatformUrl());
            urlsb.append(path);
        }
        UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(urlsb.toString());
        ParameterizedTypeReference<String> responseType = new ParameterizedTypeReference<String>() {
        };
        ResponseEntity<String> response = apiPtBomRestTemplate.exchange(builder.build().encode(StandardCharsets.UTF_8).toUri(), HttpMethod.GET, requestparam, responseType);
    ...
    public String postapi(String path, Object pms, Map<String, String> heads) {
        ...
        ResponseEntity<String> response = apiPtBomRestTemplate.exchange(builder.build().encode(StandardCharsets.UTF_8).toUri(), HttpMethod.POST, requestparam, responseType);
        String rs = response.getBody();
        ApiPlatResponseUtil.traceResponse(response);
        return rs;
    }
