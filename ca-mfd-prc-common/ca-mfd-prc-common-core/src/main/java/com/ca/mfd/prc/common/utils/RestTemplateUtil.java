...
public class RestTemplateUtil {
     ...
     * @return String
     */
    public ResponseEntity getStringForResponse(String url) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("user-agent",
                "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
                        + "(KHTML, like Gecko) Chrome/54.0.2840.99 Safari/537.36");
     ...
     * @return String
     */
    public ResponseEntity getStringForResponse(String url, Map<String, String> headpms) {
        HttpHeaders headers = new HttpHeaders();
        if (headpms == null || !headpms.containsKey("user-agent")) {
            headers.add("user-agent",
                    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
                            + "(KHTML, like Gecko) Chrome/54.0.2840.99 Safari/537.36");
        }
     ...
     * @return JSONObject
     */
    public ResponseEntity postJsonFroResponse(String url, JSONArray param, Map<String, String> header) {
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(Constant.APPLICATION_JSON_UTF8);
        ...
        HttpEntity<JSONArray> requestEntity = new HttpEntity<JSONArray>(param, headers);
        ResponseEntity responseEntity = restTemplate.postForEntity(url, requestEntity, JSONObject.class);
