...
import com.ca.mfd.prc.audit.mapper.IPqsExGradeMapper;
...
/**
 * @author inkelink
 * @Description: 精致工艺扣分等级配置服务实现
 * @date 2024年01月29日
 * @变更说明 BY inkelink At 2024年01月29日
 */
@Service
public class PqsExGradeServiceImpl extends AbstractCrudServiceImpl<IPqsExGradeMapper, PqsExGradeEntity> implements IPqsExGradeService {


    @Autowired
    private LocalCache localCache;
    private final String cacheName = "PRC_PQS_EX_GRADE";
    ...
    @Override
    public void afterDelete(Wrapper<PqsExGradeEntity> queryWrapper) {
        removeCache();
    }
    ...
    @Override
    public void afterInsert(PqsExGradeEntity model) {
        removeCache();
    }

    @Override
    public void afterUpdate(PqsExGradeEntity model) {
        removeCache();
    }

    @Override
    public void afterUpdate(Wrapper<PqsExGradeEntity> updateWrapper) {
        removeCache();
    }

    @Override
    public void beforeInsert(PqsExGradeEntity entity) {
        if (entity.isDataCheck()) {
            ...
            PqsExGradeEntity deptEntity = getData(conditionInfos).stream().findFirst().orElse(null);
            if (deptEntity != null) {
    ...
    @Override
    public void beforeUpdate(PqsExGradeEntity entity) {
        if (entity.isDataCheck()) {
            List<ConditionDto> conditionInfos = new ArrayList<>();
            conditionInfos.add(
                    new ConditionDto("GRADE_CODE", entity.getGradeCode(), ConditionOper.Equal));
    ...
    @Override
    public void saveExcelData(List<PqsExGradeEntity> entities) {
        // excel数据入库，不需要数据验证
        entities.forEach(e -> e.setDataCheck(false));

        List<PqsExGradeEntity> allDataLists = getAllDatas();
        // 需要更新的数据
        List<PqsExGradeEntity> updateList = entities.stream()
        ...
        // 需要新增的数据
        List<PqsExGradeEntity> insertList = entities.stream()
     ...
     */
    @Override
    public List<PqsExGradeEntity> getAllDatas() {
        Function<Object, ? extends List<PqsExGradeEntity>> getDataFunc = (obj) -> {
            List<PqsExGradeEntity> lst = getData(null);
            if (lst == null || lst.size() == 0) {
                return new ArrayList<>();
            }
            return lst;
        };
        List<PqsExGradeEntity> caches = localCache.getObject(cacheName, getDataFunc, -1);
        if (caches == null) {
            return new ArrayList<>();
        }
        return caches;
    ...
    public List<ComboDataDTO> getComboList() {
        List<ComboDataDTO> collect = getData(null).stream()
                .sorted(Comparator.comparing(PqsExGradeEntity::getDisplayNo)).map(c -> {
