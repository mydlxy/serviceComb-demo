 ...
 * @date 2023/4/17
 */
public interface IPqsExLogicService {


    void saveChange();

    PageData<DefectAnomalyDto> getExAnomalyShowList(DefectAnomalyParaInfo info);

    ResultVO getAuditVehicleDefectAnomalyList(GetAuditDefectAnomalyRequest request);

    ResultVO<List<PqsExEntryAttchmentEntity>> getAttchmentbyRecordNo(String recordNo);

    void saveAttachMent(PqsExEntryAttchmentEntity attchmentEntity);
     ...
     * @return
     */
    ExQgGateConfigurationInfo getQGGateData(Long qualityGateId);
    ...
    void submitQGGateData(ExQgGateConfigurationInfo info);
}
