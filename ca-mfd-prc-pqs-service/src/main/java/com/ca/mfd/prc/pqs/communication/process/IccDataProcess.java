...
@Component
//@RefreshScope
@ConditionalOnExpression("#{'true'.equals(environment.getProperty('inkelink.pqs.icc.enable'))}")
public class IccDataProcess implements ApplicationRunner, DisposableBean {
    private static final Logger logger = LoggerFactory.getLogger(IccDataProcess.class);
    /**
     * 线程执行间隔
     */
    private static final int TIME_INTERVAL = 1000 * 10;
    @Value("${inkelink.pqs.icc.isstart:1}")
    private String processIsStart;
    @Value("${inkelink.pqs.icc.enable:false}")
    ...
    private void syncData() throws InterruptedException {
        logger.info("startICC启动");
        while (true) {
            try {
                if (Boolean.FALSE.equals(enable)) {
                    logger.info("startICC停止");
                    break;
                }
                if (!"1".equals(processIsStart)) {
                    logger.info("startICC暂停");
