...
import com.ca.mfd.prc.core.report.dto.*;
...
@RestController
@RequestMapping("report/printprocess")
@Tag(name = "打印定时调度", description = "打印定时调度")
public class PrintProcessController extends BaseApiController {
    private static final Logger logger = LoggerFactory.getLogger(PrintProcessController.class);
    ...
    public ResultVO<String> excute() {
        ...
        for (Map.Entry<String, List<ReportPrinterDyc>> kv : map.entrySet()) {
            //reportPrintQueue.computeIfAbsent(kv.getKey(),v ->  new ConcurrentLinkedQueue<>()).addAll((Collection)kv.getValue());
