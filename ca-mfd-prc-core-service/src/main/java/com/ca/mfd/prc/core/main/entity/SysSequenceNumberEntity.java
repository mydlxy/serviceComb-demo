...
public class SysSequenceNumberEntity extends BaseEntity {
    ...
    private Date curMaxDate = new Date();
    ...
    public String getVinCode() {
        if (!StringUtils.isBlank(sequenceType)) {
            return sequenceType.split(",")[0].replace("SN_VIN", "").replace(",","");
        }
        return "";
    }

    public void setVinCode(String val) {
        /*if (!StringUtils.isBlank(val) && !val.toUpperCase().startsWith("SN_VIN")) {
            this.sequenceType = "SN_VIN" + val;
        }*/
        this.vinCode = val;
    }
    ...
    public String getOrderSign() {
        if (!StringUtils.isBlank(sequenceType)) {
            if (sequenceType.split(",").length > 1) {
                return sequenceType.split(",")[1].replace(",","");
            }
        }
        return "";
    }

    public void setOrderSign(String val) {
        /*if (!StringUtils.isBlank(val) && !val.toUpperCase().startsWith("SN_VIN")) {
            this.sequenceType = "SN_VIN" + val;
        }*/
        this.orderSign = val;
