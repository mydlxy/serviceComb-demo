...
import com.ca.mfd.prc.pps.remote.app.core.ISysConfigurationService;
...
public class SysConfigurationProvider {
    ...
    public List<SysConfigurationEntity> getSysConfigurationsLike(String category, String valLike) {
        ResultVO<List<SysConfigurationEntity>> result = sysConfigurationService.getSysConfigurationsLike(category, valLike);
        if (!result.getSuccess()) {
            throw new InkelinkException("服务inkelink-core-sysconfiguration调用失败" + result.getMessage());
        }
        return result.getData();
