 ...
 */
@ConditionalOnExpression("#{'true'.equals(environment.getProperty('inkelink.pps.mq.asorderscrap.enable'))}")
@Component
public class AsOrderScrapHandler {
    private static final Logger logger = LoggerFactory.getLogger(AsOrderScrapHandler.class);

    @MesRabbitListener(queues = RabbitMQQueueConstants.ADD_AS_ORDERSCRAP_QUEUE, pattern = PatternEnum.AutoRetry)
    public void addMessageQueue(Message message, Channel channel) throws IOException {
        ...
        if (rabbitMQContext != null) {
            try {//TODO 正式会删除 try..catch
                AsOrderScrapDto model = JsonUtils.parseObject(rabbitMQContext.getContent(), AsOrderScrapDto.class);
                IMidApiLogService midApiLogService = SpringContextUtils.getBean(IMidApiLogService.class);
                List<AsOrderScrapDto> datas = Arrays.asList(model);
                midApiLogService.sendOrderScrap(datas);
