...
@Component
//@RefreshScope
@ConditionalOnExpression("#{'true'.equals(environment.getProperty('inkelink.pps.asbathplan.enable'))}")
public class AsBathPlanProcess implements ApplicationRunner, DisposableBean {
    private static final Logger logger = LoggerFactory.getLogger(AsBathPlanProcess.class);
    /**
     * 线程执行间隔
     */
    private static final int TIME_INTERVAL = 1000 * 10;

    @Value("${inkelink.pps.asbathplan.isstart:1}")
    private String processIsStart;
    @Value("${inkelink.pps.asbathplan.enable:false}")
    ...
    public void startAsBathPlan() throws InterruptedException {
