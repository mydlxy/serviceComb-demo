...
import com.ca.mfd.prc.pps.remote.app.pm.IPmProductBomVersionsService;
...
public class PmProductBomVersionsProvider {
    ...
    public String copyBom(String productMaterialNo, String version) {
        ResultVO<String> result = pmProductBomVersionsService.copyBom(productMaterialNo,version);
        if (!result.getSuccess()) {
            throw new InkelinkException("服务inkelink-pm-pmproductbomversions调用失败" + result.getMessage());
        }
        return result.getData();
    ...
    public PmProductBomVersionsEntity getVersionByMaterialNo(String materialNo,String orderCategory) {
        ResultVO<PmProductBomVersionsEntity> result = pmProductBomVersionsService.getVersionByMaterialNo(materialNo,orderCategory);
        if (!result.getSuccess()) {
            throw new InkelinkException("服务inkelink-pm-pmproductbomversions调用失败" + result.getMessage());
        }
        return result.getData();
