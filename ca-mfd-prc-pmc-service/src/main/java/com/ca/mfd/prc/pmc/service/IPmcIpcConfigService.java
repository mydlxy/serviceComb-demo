package com.ca.mfd.prc.pmc.service;

import com.ca.mfd.prc.common.service.ICrudService;
import com.ca.mfd.prc.pmc.dto.IpcCameraGroupDTO;
import com.ca.mfd.prc.pmc.dto.ListCameraByGroupDTO;
import com.ca.mfd.prc.pmc.entity.PmcIpcConfigEntity;

/**
 * @author inkelink
 * @Description: 工控机配置;服务
 * @date 2023年10月26日
 * @变更说明 BY inkelink At 2023年10月26日
 */
public interface IPmcIpcConfigService extends ICrudService<PmcIpcConfigEntity> {

    IpcCameraGroupDTO listCameraByGroup(ListCameraByGroupDTO dto);


    void registerIpc(boolean enable);
}